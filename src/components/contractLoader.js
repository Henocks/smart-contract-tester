...
class contractLoader extends React.Component {
  ...
  testRunner(testList, contract) {
    let testResult = [];
    testList.forEach(async (test) => {
      //testResult.push(await Promise.resolve(contract.methods.balanceOf(test.param[0]).call()));
      eval("contract.methods." + test.func + "(test.param[0]).call().then((result) => {testResult.push(result)})");
    });
  ...
  render() {
    ...
    this.testRunner(testList, contract).then((result) => {
      console.log(result);
    });
